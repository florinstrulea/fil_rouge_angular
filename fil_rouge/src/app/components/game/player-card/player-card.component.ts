...
export class PlayerCardComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.choosePlayerSub = this.playerCardService.playerObservable$.subscribe(res=>{
      this.refreshPlayer(res);
    })

  }
  refreshPlayer(res:any){res
    this.player = res;    
    this.levelImg = "assets/player-card/level-" + res.level + ".png";
    if (this.player.id ) {
      this.levelImg = this.player.level? "assets/player-card/level-"+ this.player.level +".png": "1";
      
      /*On récupère l'ArmorEquiped si idPlayer!=null*/
      if (this.player.idArmorEquiped) {
        this.playerCardService.getCurrentArmor((this.player?.id!)).subscribe(res => {
          this.armorName = res.name != null ? res.name : "No Armor Equiped";
          this.armorEquipedImg = res.iconUrl ? "assets/equipement/armors" + res.iconUrl + ".png" : "assets/player-card/no-armor.png";
        })
      } 
      /*On récupère la WeaponEquiped si idPlayer!=null*/
      if (this.player.idWeaponEquiped) {
        this.playerCardService.getCurrentWeapon(this.player.id).subscribe(res => {
          this.weaponName = res.name  ? res.name : "No Weapon Equiped";
          this.weaponEquipedImg = res.iconUrl ? "assets/equipement/weapons" + res.iconUrl + ".png" : "assets/player-card/no-weapon.png"
